...
samples <- read.table(here("data/metadata.tsv"), header = TRUE, sep = "\t", stringsAsFactors = FALSE)
rownames(samples) <- samples$sample


counts <- read.csv(here("results/quant/gene_count_matrix.csv"), header = TRUE, row.names = 1)
